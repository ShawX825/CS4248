...
class InputExample(object):
    ...
    def __init__(self, guid, text_a, text_b=None, label=None, text_c=None, text_e=None):
...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids, label_id, speaker_ids, mention_ids, topic_ids):
...
class bertsProcessor(DataProcessor): #bert_s
    ...
    def _create_examples(self, data, set_type):
        ...
        for (i, d) in enumerate(data):
            guid = "%s-%s" % (set_type, i)
            text_a = data[i][0] # data 
            text_b = data[i][1] # person 1
            text_c = data[i][2] # person 2
            text_d = data[i][3] # label
            text_e = data[i][4] # topic
            examples.append(InputExample(guid=guid, text_a=text_a, text_b=text_b, label=text_d, text_c=text_c, text_e = text_e))
...
class bertsf1cProcessor(DataProcessor): #bert_s (conversational f1)
    ...
    def _create_examples(self, data, set_type):
        ...
        for (i, d) in enumerate(data):
            ...
            text_c = data[i][2]
            text_e = data[i][4]
            examples.append(InputExample(guid=guid, text_a=text_a, text_b=text_b, label=data[i][3], text_c=text_c, text_e = text_e))
            
        return examples

def tokenize(text, tokenizer, start_mention_id, topic_list):
    ...
    for i, t in enumerate(textraw):
        if t in ['speaker 1', 'speaker 2', 'speaker 3', 'speaker 4', 'speaker 5', 'speaker 6', 'speaker 7', 'speaker 8', 'speaker 9']:
            ...
            for tok in tokens:
                ...
                topic_ids.append(topic_list[min(mention_id-1, len(topic_list)-1)] + 1)
    ...
    while len(topic_ids) < len(mention_ids):
        topic_ids.append(0)
    assert(len(topic_ids) == len(mention_ids))
    return text, speaker_ids, mention_ids, topic_ids 
...
def convert_examples_to_features(examples, max_seq_length, tokenizer):
    ...
    for (ex_index, example) in enumerate(examples):
        tokens_a, tokens_a_speaker_ids, tokens_a_mention_ids, tokens_a_topic_ids = tokenize(example.text_a, tokenizer, 0,  example.text_e)
        #print("convert examples to features: topic ids: ", tokens_a_topic_ids[:10])
        ...
        topic_ids = topic_ids + tokens_a_topic_ids
        ...
        topic_ids = topic_ids + [0] * len(tokens_b)
        ...
        while len(input_ids) < max_seq_length:
            ...
            mention_ids.append(0)
            topic_ids.append(0)
        while len(topic_ids) < max_seq_length:
            topic_ids.append(0)
        topic_ids = topic_ids[:max_seq_length]
        assert len(input_ids) == max_seq_length
        assert len(input_mask) == max_seq_length
        assert len(segment_ids) == max_seq_length
        assert len(speaker_ids) == max_seq_length
        assert len(mention_ids) == max_seq_length
        assert len(topic_ids) == max_seq_length
        ...
        if ex_index < 5:
            ...
            logger.info(
                    "topic_ids: %s" % " ".join([str(x) for x in topic_ids]))
    ...
    def __init__(self, src_file, save_file, max_seq_length, tokenizer, n_class, encoder_type):
        ...
        if os.path.exists(save_file):
            ...
            for f in features:
                ...
                #print("dataset saved: ", f[0].topic_ids[:10])
                self.data.append({
                    ...
                    'topic_ids':np.array(f[0].topic_ids),
...
class TUCOREGCNDataloader(DataLoader):
    ...
    def __iter__(self):
        ...
        topic_ids = torch.LongTensor(self.batch_size, self.max_length).cpu()
        ...
        for idx, minibatch in enumerate(self.batches):
            ...
            for i, example in enumerate(minibatch):
                mini_input_ids, mini_segment_ids, mini_input_mask, mini_label_ids, mini_mention_id, mini_speaker_id, turn_mask, graph, mini_topic_id = \
                    example['input_ids'], example['segment_ids'], example['input_mask'], example['label_ids'], \
                    example['mention_id'], example['speaker_ids'], example['turn_mask'], example['graph'], example['topic_ids']
                ...
                topic_ids[i: word_num].copy_(torch.from_numpy(mini_topic_id))
            ...
            yield {'input_ids': get_cuda(input_ids[:cur_bsz, :batch_max_length].contiguous()),
                   ...
                   'graphs': graph_list,
                   'topic_ids':get_cuda(topic_ids[:cur_bsz, :batch_max_length].contiguous()),
