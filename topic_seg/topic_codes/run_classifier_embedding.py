...
from models.BERT.TUCOREGCN_BERT_embedding import BertConfig, TUCOREGCN_BERT
...
from data_embedding import TUCOREGCNDataset, TUCOREGCNDataloader, TUCOREGCNDataset4f1c
...
def get_logits4eval(model, dataloader, savefile, device):
    ...
    for batch in tqdm(dataloader, desc="Iteration"):
        ...
        with torch.no_grad():
            tmp_eval_loss, logits = model(input_ids=input_ids, token_type_ids=segment_ids, attention_mask=input_masks, speaker_ids=speaker_ids, graphs=graphs, mention_id=mention_ids, labels=label_ids, turn_mask=turn_mask, topic_ids = topic_ids)
    ...
    if args.do_train:
        ...
        for epoch in tqdm(trange(int(args.num_train_epochs)), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_loader, desc="Iteration")):
                ...
                loss, _ = model(input_ids=input_ids, token_type_ids=segment_ids, attention_mask=input_masks, speaker_ids=speaker_ids, graphs=graphs, mention_id=mention_ids, labels=label_ids, turn_mask=turn_mask, topic_ids=topic_ids)
                if n_gpu > 1:
                    loss = loss.mean()
