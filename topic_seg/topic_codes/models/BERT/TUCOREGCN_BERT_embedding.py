...
class BertConfig(object):
    ...
    def __init__(self,
        ...
        self.max_topic_embeddings = max_topic_embeddings
...
class BERTEmbeddings(nn.Module):
    def __init__(self, config):
        ...
        self.topic_embeddings = nn.Embedding(config.max_topic_embeddings, config.hidden_size)
    ...
    def forward(self, input_ids, speaker_ids,  topic_ids, token_type_ids=None):
        ...
        import numpy as np
        #print("example topic ids: ", topic_ids[2:])
        topic_embeddings = self.topic_embeddings(topic_ids)#topic_ids)
        embeddings = words_embeddings + position_embeddings + token_type_embeddings + speaker_embeddings + position_embeddings + topic_embeddings
        #print("1.embedding size after add topic", str(embeddings.size()))
        embeddings = self.LayerNorm(embeddings)
        #print("2.embedding size after layernorm in embedding layer", str(embeddings.size()))
        
        embeddings = self.dropout(embeddings)
        #print("3.embedding size after dropout in embedding layer", str(embeddings.size()))
...
class BERTAttention(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask):
        #print("16, reached attention before", str(attention_mask.size()), str(input_tensor.size()))
        self_output = self.self(input_tensor, attention_mask)
        attention_output = self.output(self_output, input_tensor)
        #print("12, attention after", str(attention_output.size()))
...
class BERTLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask):
        #print("14. reached bertlayer, ?, hidden state, attention mask", str(hidden_states.size()), str(attention_mask.size()))
        attention_output = self.attention(hidden_states, attention_mask)
        #print("11. attention ", str(attention_output.size()))
...
class BERTEncoder(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask):
        all_encoder_layers = []
        #print("13. reached encoder, ?", str(hidden_states.size()), str(attention_mask.size()))
        
        for layer_module in self.layer:
            hidden_states = layer_module(hidden_states, attention_mask)
            all_encoder_layers.append(hidden_states)
        #print("10. encoder size, ", str(len(all_encoder_layers)), str(all_encoder_layers[0].size()))
...
class BertModel(nn.Module):
    ...
    def forward(self, input_ids, speaker_ids, topic_ids, token_type_ids=None, attention_mask=None):
        ...
        embedding_output = self.embeddings(input_ids, speaker_ids,  topic_ids, token_type_ids)
        all_encoder_layers = self.encoder(embedding_output, extended_attention_mask)
        #print("4.embedding size after encoded in bertmodel layer {}, {}", str(len(all_encoder_layers)), str(all_encoder_layers[0].size()))
        
        sequence_output = all_encoder_layers[-1]
        #print("5.embedding size after taking last layer in bertmodel layer", str(sequence_output.size()))
        
        pooled_output = self.pooler(sequence_output)
        #print("6.embedding size after being pooled in bertmodel layer", str(pooled_output.size()))
...
class TUCOREGCN_BERT(nn.Module):
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, speaker_ids, graphs, mention_id, topic_ids, labels=None, turn_mask=None):
        slen = input_ids.size(1)
        #print("100. attention mask size? ", str(attention_mask.size()))
        #print("topic ids at gcn bert:", topic_ids[:10])
        sequence_outputs, pooled_outputs = self.bert(input_ids, speaker_ids, topic_ids, token_type_ids, attention_mask) 
        
        features = None
        sequence_outputs, _ = self.turnAttention(sequence_outputs, sequence_outputs, sequence_outputs, turn_mask)
        #print("7. embedding after attention layer ", str(sequence_outputs.size()))
        num_batch_turn = []
        ...
        #print("8. after graph output, in gcn, ", str(pooled_output.size()))
        logits = self.classifier(pooled_output)
        logits = logits.view(-1, self.num_labels)
        #print("9. after classifier, ", str(logits.size()))
