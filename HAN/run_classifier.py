...
def get_logits4eval(model, dataloader, savefile, device):
    ...
    for batch in tqdm(dataloader, desc="Iteration"):
        ...
        with torch.no_grad():
            tmp_eval_loss, logits, batch_semantic_attn, batch_node_attn = model(input_ids=input_ids, token_type_ids=segment_ids, attention_mask=input_masks, speaker_ids=speaker_ids, graphs=graphs, mention_id=mention_ids, labels=label_ids, turn_mask=turn_mask, get_node_attention=True, get_semantic_attention=True)
        ...
        if "logits_test.txt" in savefile:
            semantic_attn_all += [ [batch['label_ids'][i], pred[i], attn] for i, attn in enumerate(batch_semantic_attn)]
            # for i in range(len(node_attn)):
            node_attn_all += batch_node_attn 
    ...
    if "logits_test.txt" in savefile:
        ...
        with open(savefile, 'wb') as f:
            pickle.dump([semantic_attn_all,node_attn_all],f)
...
def get_logits4eval_ERC(model, dataloader, savefile, resultsavefile, device, data_name):
    ...
    for batch in tqdm(dataloader, desc="Iteration"):
        ...
        with torch.no_grad():
            tmp_eval_loss, logits, _, _ = model(input_ids=input_ids, token_type_ids=segment_ids, attention_mask=input_masks, speaker_ids=speaker_ids, graphs=graphs, mention_id=mention_ids, labels=label_ids, turn_mask=turn_mask)
    ...
    if args.do_train:
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_loader, desc="Iteration")):
                ...
                loss, _, _, _ = model(input_ids=input_ids, token_type_ids=segment_ids, attention_mask=input_masks, speaker_ids=speaker_ids, graphs=graphs, mention_id=mention_ids, labels=label_ids, turn_mask=turn_mask)
                if n_gpu > 1:
                    loss = loss.mean()
